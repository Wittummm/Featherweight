...
void main() {
	...
	vec3 screenPos = (clipPos.xyz / clipPos.w)*0.5 + 0.5;
		...
		float deepness = terrainDepth < screenPos.z ? 0.8 : clamp(distance(viewPos, depthToViewPos(fragCoord, terrainDepth))*0.05, 0, 1);
		...
		const float noiseSpeed = frameTimeCounter*waveSpeed*0.5 + frameTimeCounter*waveSpeedMagic*0.5;
