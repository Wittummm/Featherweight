...
vec3 ssr(vec3 startPos, vec3 endPos) {
    ...
    if (endFrag.z <= 1) {
        for (float i = 0; i < SSR_STEPS; i++) {
            ...
            currentFrag.z = linearizeDepth(currentFrag.z);
            float currentDepth = linearizeDepth(texture(depthtex0, currentFrag.xy).r) + bias;
            ...
            if (currentFrag.z > currentDepth && currentFrag.z - currentDepth < maxThickness) {
                ...
                for (float j = 0; j < SSR_REFINE_STEPS; j++) {
                    ...
                    float BcurrentDepth = linearizeDepth(texture(depthtex0, BcurrentFrag.xy).r) + bias;
...
void main() {
    ...
    ivec2 pixelCoord = ivec2(fragCoord*vec2(viewWidth, viewHeight));
        ...
        float depth;
        vec3 normalsImplied = normalize(reconstructNormals(pixelCoord/vec2(viewWidth, viewHeight), depthtex0, depth));
    #else
