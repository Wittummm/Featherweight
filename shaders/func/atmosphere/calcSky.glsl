...
vec3 calcSky(vec3 dir, vec2 intensity, vec2 attenuation) {
    return skyCustomNoStars(mat3(gbufferModelViewInverse) * dir, intensity, attenuation); 
}

vec3 calcSky(vec3 dir) {
    return skyCustomNoStars(mat3(gbufferModelViewInverse) * dir); 
}

vec3 calcSkyWithStars(vec3 dir) {
    return skyCustom(mat3(gbufferModelViewInverse) * dir); 
}
