...
function toneMapPicker(name, def) {
  return asIntRange(name, def != null ? def : 2, 0, 5, 1, false).name().values("NONE", "Reinhard+", "AgX", "Heij", "Unreal", "Lottes").desc("[NONE] is no tonemapping, not recommended.");
}
...
function setupOptions() {
  ...
  const shadow = new Page("Page_Shadows").name().add(asBool("ShadowsEnabled", true, true).name()).add(
    ...
    new Page("Page_ShadowsExtra").name().add(shadowPixelization).add(asFloatRange("ShadowDistort", 3, 0, 6, 0.05, false).name().desc("Allocate more resolution closer to the player.\n When using Shadow Cascades, this value should generally be lowered.")).add(asIntRange("ShadowCascadeCount", 0, 0, 8, 1, true).name().values("Auto")).add(asFloatRange("ShadowBias", 0.6, 0, 1, 0.025, false).name().desc("Increase to reduce shadow acne at the cost of increasing peter panning.")).add(asFloatRange("ShadowThreshold", 0, 0, 1, 0.05, false).name().desc('How shadowed the shadow needs to be to be considered shadowed, higher will "shrink" shadows.')).build()
  ...
  const cameraAndColors = new Page("Page_CameraAndColors").name().add(
    new Page("Exposure").add(asFloatRange("ExposureMult", 1, -2, 5, 0.05, false).name()).add(asIntRange("AutoExposure", 1, 0, 1, 1, canReload).name().values("Off", "Low")).add(asFloatRange("ExposureSpeed", 5, 0, 20, 0.1, false).name()).add(asIntRange("ExposureSamplesX", 12, 1, 32, 1, true).name()).build()
  ).add(
    new Page("Page_Tonemapping").name().add(toneMapPicker("Tonemap")).add(asBool("CompareTonemaps", false, false).name()).add(toneMapPicker("Tonemap1", 1)).add(toneMapPicker("Tonemap2", 2)).add(toneMapPicker("Tonemap3", 3)).add(toneMapPicker("Tonemap4", 4)).build()
  ).build();
  const debug = new Page("_Debug").name().add(asBool("_DumpTags", false, false).name()).add(asBool("_DumpUniforms", false, false).name()).add(asBool("_DebugEnabled", false, shouldReload).name()).add(
