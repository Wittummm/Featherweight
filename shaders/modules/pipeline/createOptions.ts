const canReload = true; // Works without needing to reload, reloading is better than not, but is not significant (default: true)
...
export default function createOptions() {
    ...
    .add(new Page("Page_LightShafts").name()
        .add(new Page("Page_Sunrays").name()
            .add(asBool("SunraysEnabled", true, canReload).name())
            .add(asFloatRange("SunraysStrength", 0.5, 0, 2, 0.05, false).name())
            .add(asFloatRange("SunraysSpread", 0.75, 0.05, 1, 0.01, false).name())
            .add(asFloatRange("SunraysOriginSize", 0.3, 0, 1, 0.05, false).name())

            .add(asFloatRange("SunraysSamples", 2, 0.5, 6, 0.05, false).name())
            .add(asFloatRange("SunraysFakeSamples", 1, 0, 5, 0.5, false).name())

            .add(new Page("Page_SunraysExtra").name()
                .add(asInt("SunraysSamplesOverride", -1, 10, 20, 30, 40, 50).needsReload(false).build(-1).name().values("Off"))
                .build()
            )
            .build()
        )
